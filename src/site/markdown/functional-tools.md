...
Finds fully qualified Java class names whose **simple (short) names** match a Java regular expression. Use this tool when you know a class_name pattern but do not know the package, such as when looking for service, controller, or Maven Mojo implementations. It is commonly the discovery step before `get_class_info`.
...
- Uses the same cached project scan as `find_class`, so the metadata reflects the state when that scan was first initialized.
...
- `find_class` initializes (and then searches) a cached classpath scan and matches only simple names.
